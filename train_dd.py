...
from dataloader_10 import  get_chunk, EmoryNLPDataset, MELDDataset
...
def get_loader_meld(path, batch_size=1, valid=0.1, num_workers=8, MAX_L=20, num_class = 7, model_type='roberta_large', pin_memory=True, cuda_=True):
...
def get_loader_emory(path, batch_size=1, num_workers=2, MAX_L=20, model_type='roberta_large', n_classes=7, pin_memory=False, cuda_=False):
...
# 训练或评估模型的函数
def train_or_eval_model(model, loss_Func, dataloader, epoch, optimizer=None, scheduler = None, model_type='roberta_large',
    ...
    for data in tqdm(dataloader):
        ...
        #原始👇
        if data_type == 'meld':
            sent_ids, mask, token_types, occu_ids, occu_masks, occu_token_types,styletopic_ids, styletopic_masks, styletopic_token_types, cpt_graph_i, _, speakers, umask, label, \
             str_src, str_dst, str_edge_type,timestr_src, timestr_dst, timestr_edge_type = [d.cuda() if torch.is_tensor(d) else d for d in data[:-1]] if cuda_ else data[:-1]
        #原始👆

        elif data_type == 'emory':
            sent_ids, mask, token_types, occu_ids,occu_masks, occu_token_types,styletopic_ids, styletopic_masks, styletopic_token_types,cpt_graph_i, speakers, umask, label, \
            str_src, str_dst, str_edge_type ,timestr_src, timestr_dst, timestr_edge_type= \
        ...
        inputsoccupation = {'input_ids': occu_ids, 'attention_mask': occu_masks, 'token_type_ids': occu_token_types}
        inputstyletopic = {'input_ids':styletopic_ids, 'attention_mask': styletopic_masks, 'token_type_ids': styletopic_token_types}
        labels_ = label.view(-1)
        log_prob, loss = model.forward(inputs, inputsoccupation,inputstyletopic, str_src, str_dst, str_edge_type,timestr_src, timestr_dst, timestr_edge_type,cpt_graph_i, labels_,loss_func=loss_Func, train=train)
...
if __name__ == '__main__':
    ...
    else:loss_weights = torch.FloatTensor([1.0, 1.0, 1.0])
#    根据数据类型和关系数量获取数据加载器
    ...
    if Configs.data_type == 'meld':
        best_va_fscore, best_fscore, best_loss, best_label, best_pred, best_mask, best_va_loss = None, None, None, None, None, None, None
        patience = 0
        for e in range(Configs.epochs):
            ...
            if best_va_loss == None or best_va_loss >= valid_loss:
        ...
        for e in range(Configs.epochs):
            ...
            print(
                ...
                format(e + 1, train_loss, train_acc, train_fscore, valid_loss, valid_acc, valid_fscore, test_loss,
                       test_acc, test_fscore, round(time.time() - start_time, 2)))
        print('Test performance..')
        print('Fscore {} accuracy {}'.format(best_fscore,round(accuracy_score(best_label, best_pred, sample_weight=best_mask) * 100,2)))
        print(classification_report(best_label, best_pred, sample_weight=best_mask, digits=4))
        print(confusion_matrix(best_label, best_pred, sample_weight=best_mask))
